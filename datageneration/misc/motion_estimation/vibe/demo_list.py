"""
MODIFIED VERSION THAT SUPPORTS A LIST OF VIDEOS
  loading the pretrained model only once

  e.g. usage:
    conda activate vibe-env

    # Demo
    python demo_list.py --vid_files_list sample_video_list.txt --output_folder output_list --no_render

    # NTU (Run for train/test 0/45/90)
    python demo_list.py --vid_files_list /sequoia/data1/gvarol/similarity/render_actions/vidlists/ntu_cvs_split/cvs_train0.txt \
        --output_folder /home/gvarol/datasets/ntu/vibe/train/ --no_render --video_extension .avi

    # UESTC
    python demo_list.py --vid_files_list /home/gvarol/datasets/uesic/25600seq_all.txt --output_folder /home/gvarol/datasets/uestc/vibe/ --no_render --video_extension .avi
...
def main(args):
    ...
    with open(args.vid_files_list, "r") as f:
        video_files = f.read().splitlines()

    for video_file in video_files:
        run_on_single_video(args, video_file, model, device, ext=args.video_extension)


def run_on_single_video(args, video_file, model, device, ext=".mp4"):
    ...
    output_path = os.path.join(args.output_folder, os.path.basename(video_file).replace(ext, ''))
    if os.path.exists(os.path.join(output_path, "vibe_output.pkl")):
        print(f"VIBE: Already extracted at {output_path}")
        return
    ...
    if not args.no_render:
        ...
        save_name = f'{vid_name.replace(ext, "")}_vibe_result.mp4'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vid_files_list', type=str,
                        help='text file with paths to input videos or youtube links')
