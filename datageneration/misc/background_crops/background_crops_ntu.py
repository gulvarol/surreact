...
def main():
    ...
    backgrounds_root = "/home/gvarol/datasets/ntu/backgrounds/"
    ...
    # Loop over the data
    for j in range(0, len(ntu_data), 1):
        if setname == "train":
        ...
        # If the video is a 1-person action
        if ntu_data.actions[ind] < 49:
            # print(ntu_data.videos[ind])
            T = ntu_data.num_frames[ind]
            rand_t = np.random.randint(T)
            # RGB image (3, 1080, 1920)
            rgb_full = ntu_data._load_rgb(ind, t=rand_t)
            # (1080, 1920, 3)
            ...
            for i in range(num_back):
                ...
                if back_width > 300 and new_height > 240:
                    ...
                    img = scipy.misc.imresize(
                        img_background[i], [new_height, crop_width]
                    )
                    ...
                    print("Saving img to {}".format(filejpg))
