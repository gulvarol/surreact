...
for JOBID in {0..399}; do echo "${JOBID}"; done > $CODESH
...
do
  ...
  do
    ./run.sh '--idx ${JOBID} \
      --zrot_euler '\"\$v\"' \
      --repetition '\"\$r\"' \
      --vidlist_path vidlists/uestc/400seq_surreact_hmmr_subset.txt \
      --smpl_result_path ../data/uestc/hmmr/train \
      --smpl_estimation_method hmmr \
      --bg_path ../data/uestc/backgrounds/ \
      --output_path ../data/surreact/uestc/hmmr/ \
      --tmp_path ../data/surreact/uestc/tmp_hmmr_output/ \
      --datasetname uestc \
      --split_name train \
